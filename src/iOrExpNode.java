 ...
 */
public class iOrExpNode extends ExpNode{
    iOrExpNode(ExpNode ls,ExpNode rs){
        this.lsNode = ls;
        this.rsNode = rs;
    }
    ...
    void visit3(mxxTable table,int scope){
        ...
        if	(this.lsNode.calcRes.depth > 0){
            this.ok = 0;
            System.out.println("too much deps for |");
        }
    ...
    Address buildExp(List<Quadruple> quadList, Map<String,Integer> getSize,mxxTable table,int scope){
        ...
        Temp tmp = new Temp();
        quadList.add(new ArithmeticExpr(tmp,lef,ArithmeticOp.OR,rig));
        return  tmp;
    }
}