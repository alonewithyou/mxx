...
class andExpNode extends ExpNode{
    andExpNode(ExpNode ls,ExpNode rs){
        this.lsNode = ls;
        this.rsNode = rs;
    }
    ...
    void visit3(mxxTable table,int scope){
        ...
        if	(!this.lsNode.calcRes.typeID.equals("int")){
            this.ok = 0;
            System.out.println("expecting int &");
        }
    ...
    Address buildExp(List<Quadruple> quadList, Map<String,Integer> getSize,mxxTable table,int scope){
        ...
        Temp tmp = new Temp();
        quadList.add(new ArithmeticExpr(tmp,lef,ArithmeticOp.AND,rig));
        return  tmp;
    }
}