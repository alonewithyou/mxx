...
public class logicOrExpNode extends ExpNode{
    logicOrExpNode(ExpNode ls,ExpNode rs){
        this.lsNode = ls;
        this.rsNode = rs;
    }
    ...
    Address buildExp(List<Quadruple> quadList,Map<String,Integer> getSize,mxxTable table,int scope){
        ...
        Label alreadytrue = new Label(), needcalc = new Label(), expover = new Label();
        quadList.add(new Br(lef,alreadytrue,needcalc));
        ...
        quadList.add(alreadytrue);
        quadList.add(new Assign(tmp,new IntegerConst(1)));
        quadList.add(new Jump(expover));
